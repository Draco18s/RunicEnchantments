...
| `̹` `͗` | Substack modifier | When applied to a stack operator (`}` `{` `S` `s` `r` `l` but not `o`) this modifier applies the command to the stack-of-stacks instead (See `[`). Current active stack is considered the top.<br/> When applied to a Fork command (`I` `J` `H` `L`) the active IP's current stack will be duplicated to the new IP |
